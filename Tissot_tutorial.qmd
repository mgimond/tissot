...
> Note 1: In this tutorial, PROJ string syntax is used to define projections in this tutorial. This may generate warnings when running some R scripts given that WKT format is now becoming a preferred projection format.

> Note 2: Modifying a projection's tangent or secant coordinate values may result in distorted base maps and/or errors in the underlying modfied geometry.
...
The Mercator projection is conformal in that the shape of the circle remains a circle. Also, its lines of longitude and latitude remain at right angles across the entire map extent. Its area and scale, however, are heavily distorted as one approaches the poles.
...
We'll first create a custom function whereby areal scales less than one will be assigned a red hue, areal scales greater than one will be assigned a blueish hue, and scales near one will be assigned a yellow hue.
...
Recall that the green lines represent the largest scale measurement and the red line the smallest scale measurement. The distances seem to be consistent at all directions.

We can extract the length of the principal directions from the `tsf` object. For the maximum scale distortion lengths for each indicatrix, type:
...
The output units are those of the projection mapping units--meters in this example.
...
To preserve distance scales across the map's extent, area scale needs to be compromised. Its values can be extracted as follows:

```{r}
params$area
```

The area scale ranges from `r round(min(params$area),2)` to `r round(max(params$area),2)` within the bounds defined by `ll` and `ur.`

------------------------------------------------------------------------

*All code in this tutorial were created with the following external libraries* `r knitr::kable(sf_extSoftVersion(), col.names = NULL)`
