...
> Note that PROJ string syntax is used to define projections in this tutorial. This may generate warnings when running some R scripts given that WKT format is now becoming a preferred projection format.
...
ggplot() + 
  geom_sf(data = world.merc, fill = "bisque", col = "grey20") + 
  geom_sf(data = tsf$base, fill = NA, col = "grey50") +
  geom_sf(data = tsf$ind,  col="red", fill = NA) +
  geom_sf(data = tsf$mina,  col="red", fill = NA) +
  geom_sf(data = tsf$maja,  col="green", fill = NA) +
...
At a latitude of 10° (20° of the southern-most standard line) the areal scale remains unity, the length scale varies from `r round(ti.aea$min.scale,3)` to `r round(ti.aea$max.scale,3)` 1.086 depending on the direction the distance is measured at.
